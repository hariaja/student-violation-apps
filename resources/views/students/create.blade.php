...
<div class="block block-rounded">
  ...
  <div class="block-content block-content-full">
    ...
    <form action="{{ route('students.store') }}" method="POST" onsubmit="return disableSubmitButton()">
      @csrf
      ...
      <div class="row justify-content-center">
        <div class="col-md-6">
          ...
          <div class="mb-4">
            ...
            <input type="text" name="name" id="name" value="{{ old('name') }}" class="form-control @error('name') is-invalid @enderror" placeholder="{{ trans('Nama Siswa') }}" onkeypress="return onlyLetter(event)">
            @error('name')
          ...
          <div class="mb-4">
            ...
            <select name="room_id" id="room_id" class="js-select2 form-select @error('room_id') is-invalid @enderror" data-placeholder="{{ trans('Pilih Kelas') }}" style="width: 100%;">
              ...
              <option value="{{ $item->id }}" @if (old('room_id')==$item->id) selected @endif>{{ $item->name }}</option>
              @endforeach
            </select>
            @error('room_id')
            <div class="invalid-feedback">{{ $message }}</div>
          ...
          <div class="mb-4">
            ...
            <span class="text-danger">*</span>
            <input type="text" name="father" id="father" value="{{ old('father') }}" class="form-control @error('father') is-invalid @enderror" placeholder="{{ trans('Nama Ayah') }}" onkeypress="return onlyLetter(event)">
            @error('father')
            <div class="invalid-feedback">{{ $message }}</div>
            @enderror
          </div>
          ...
          <div class="mb-4">
            ...
            <span class="text-danger">*</span>
            <input type="text" name="mother" id="mother" value="{{ old('mother') }}" class="form-control @error('mother') is-invalid @enderror" placeholder="{{ trans('Nama Ibu') }}" onkeypress="return onlyLetter(event)">
            @error('mother')
            <div class="invalid-feedback">{{ $message }}</div>
            @enderror
          </div>
          ...
          <div class="mb-4">
            ...
            <span class="text-danger">*</span>
            <input type="text" name="phone" id="phone" class="form-control form-control-lg @error('phone') is-invalid @enderror" value="{{ old('phone') }}" oninput="formatPhoneNumber()" placeholder="{{ trans('Masukkan no. Telepon') }}">
            @error('phone')
            <div class="invalid-feedback">{{ $message }}</div>
            @enderror
          </div>
          ...
          <div class="mb-4">
            ...
            <span class="text-danger">*</span>
            <textarea name="address" id="address" style="height: 130px" class="form-control @error('address') is-invalid @enderror" placeholder="{{ trans('Alamat Lengkap') }}">{{ old('address') }}</textarea>
            @error('address')
            <div class="invalid-feedback">{{ $message }}</div>
            @enderror
          </div>
